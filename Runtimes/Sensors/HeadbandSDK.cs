...
namespace NeuroSDK
{
    ...
    public class HeadbandSensor : Sensor
    {
        private IntPtr _this;

        private IntPtr _resistCallbackHeadbandHandle;
        private IntPtr _signalDataCallbackHeadbandHandle;
        private IntPtr _fpgDataCallbackHandle;
        private IntPtr _modeCallbackNeuroSmartHandle;
        private IntPtr _memsDataCallbackHandle;

        private readonly ResistCallbackHeadbandSensor _resistCallbackHeadband;
        private readonly SignalDataCallbackHeadbandSensor _signalDataCallbackHeadband;
...
#pragma warning disable CS8632 // The annotation for nullable reference types should only be used in code within a '#nullable' annotations context.
        public event HeadbandResistDataRecived? EventHeadbandResistDataRecived;
        public event HeadbandSignalDataRecived? EventHeadbandSignalDataRecived;
...
#pragma warning restore CS8632 // The annotation for nullable reference types should only be used in code within a '#nullable' annotations context.
        ...
        internal HeadbandSensor(IntPtr sensorPtr) : base(sensorPtr)
        {
            _this = GCHandle.ToIntPtr(GCHandle.Alloc(this));
            OpStatus opSt;

            byte error = 0;
            _resistCallbackHeadband = ResistCallbackHeadband;
            if (IsSupportedFeature(SensorFeature.FeatureResist))
            {
                error = SDKApiFactory.Inst.AddResistCallbackHeadband(_sensorPtr, _resistCallbackHeadband, out _resistCallbackHeadbandHandle, _this, out opSt);
                SDKApiFactory.ThrowIfError(opSt, error);
            }
            _signalDataCallbackHeadband = SignalDataCallbackHeadband;
            if (IsSupportedFeature(SensorFeature.FeatureSignal))
            {
                error = SDKApiFactory.Inst.AddSignalDataCallbackHeadband(_sensorPtr, _signalDataCallbackHeadband, out _signalDataCallbackHeadbandHandle, _this, out opSt);
                SDKApiFactory.ThrowIfError(opSt, error);
            }
...
#pragma warning disable CS8632 // The annotation for nullable reference types should only be used in code within a '#nullable' annotations context.
        private static HeadbandSensor? SDKSensor(IntPtr userData)
        {
            if (userData != IntPtr.Zero)
            {
                ...
                {
                    return handle.Target as HeadbandSensor;
                }
            }
            return null;
        }
#pragma warning restore CS8632 // The annotation for nullable reference types should only be used in code within a '#nullable' annotations context.
        ...
        public override void Dispose()
        {
            if (!_disposed)
            {
                SDKApiFactory.Inst.RemoveResistCallbackHeadband(_resistCallbackHeadbandHandle);
                SDKApiFactory.Inst.RemoveSignalDataCallbackHeadband(_signalDataCallbackHeadbandHandle);
        ...
        [AOT.MonoPInvokeCallback(typeof(ResistCallbackHeadbandSensor))]
        private static void ResistCallbackHeadband(IntPtr ptr, HeadbandResistData data, IntPtr userData)
        {
            var sensor = SDKSensor(userData);
            sensor?.EventHeadbandResistDataRecived?.Invoke(sensor, data);
        }
        [AOT.MonoPInvokeCallback(typeof(SignalDataCallbackHeadbandSensor))]
        private static void SignalDataCallbackHeadband(IntPtr ptr, HeadbandSignalData[] dataArray, int dataSize, IntPtr userData)
        {
            var sensor = SDKSensor(userData);
            sensor?.EventHeadbandSignalDataRecived?.Invoke(sensor, dataArray);
        }
