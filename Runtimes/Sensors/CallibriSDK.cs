...
#pragma warning restore CS8632 // The annotation for nullable reference types should only be used in code within a '#nullable' annotations context.
        ...
        public IReadOnlyList<SensorFilter> SupportedFilters
        {
            get
            {
                SensorFilter[] arr;
                OpStatus opSt;
                byte error = SDKApiFactory.Inst.GetSupportedFiltersSensor(_sensorPtr, out arr, out opSt);
                SDKApiFactory.ThrowIfError(opSt, error);
                return arr;
            }
        }
        ...
        public SensorSamplingFrequency SamplingFrequencyResp
        {
            get
            {
                SensorSamplingFrequency val;
                OpStatus opSt;
                byte error = SDKApiFactory.Inst.ReadSamplingFrequencyRespSensor(_sensorPtr, out val, out opSt);
                SDKApiFactory.ThrowIfError(opSt, error);
                return val;
            }
        }
        public SensorSamplingFrequency SamplingFrequencyEnvelope
        {
            get
            {
                SensorSamplingFrequency val;
                OpStatus opSt;
                byte error = SDKApiFactory.Inst.ReadSamplingFrequencyEnvelopeSensor(_sensorPtr, out val, out opSt);
                SDKApiFactory.ThrowIfError(opSt, error);
                return val;
            }
        }
        ...
        public CallibriElectrodeState ElectrodeState
        {
            get
            {
                CallibriElectrodeState val;
                OpStatus opSt;
                byte error = SDKApiFactory.Inst.ReadElectrodeStateCallibri(_sensorPtr, out val, out opSt);
                SDKApiFactory.ThrowIfError(opSt, error);
                return val;
            }
        }
        public bool IsSupportedFilter(SensorFilter filter)
        {
            return SDKApiFactory.Inst.IsSupportedFilterSensor(_sensorPtr, filter) == 1;
        }
        public static CallibriColorType ReadColorInfo(SensorInfo sensorInfo)
        {
            CallibriColorType color;
            SDKApiFactory.Inst.ReadColorInfo(sensorInfo, out color);
            return color;
        }
